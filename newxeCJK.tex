...
\msg_new:nnnn { newxeCJK } { charclass-already-defined }
  { XeTeX~charclass~#1~already~defined. }
  {
    newxeCJK~has~been~asked~to~create~a~new~charclass~'#1',\\
    but~this~name~has~already~been~used~elsewhere.
  }
\msg_new:nnnn { newxeCJK } { charclass-not-defined }
  { XeTeX~charclass~#1~undefined. }
  {
    newxeCJK~has~been~asked~to~use~a~charclass~'#1',\\
    but~this~has~not~been~defined~yet.
  }
\cs_new_protected:Npn \@@_chk_if_exist_charclass:n #1
  {
    \cs_if_exist:cF { g_@@_ #1 _class }
      {
        \msg_error:nnx { newxeCJK } { charclass-not-defined }
          { \tl_to_str:n {#1} }
      }
  }

% Creating new charclasses

\cs_new_protected:Npn \@@_charclass_new:n #1
  {
    \cs_if_exist:cTF { g_@@_ #1 _class }
      {
        \msg_error:nnx { newxeCJK } { charclass-already-defined }
          { \tl_to_str:n {#1} }
      }
      {
...
\@@_charclass_new:n { CJK }
\@@_charclass_new:n { Closing_LeftHalf }
\@@_charclass_new:n { Closing_MiddleHalf }
\@@_charclass_new:n { Closing_Full }
\@@_charclass_new:n { Opening_RightHalf }
\@@_charclass_new:n { Opening_Full }
\@@_charclass_new:n { non_CJK_Punct }
...
\cs_new_protected:Npn \SetPunctMinWidthRatio #1
  {
    ...
    \fp_set:Nn \l_@@_punct_removable_width_ratio_fp
      { 1 - (#1) }
  }
...
\cs_new_protected:Npn \SetMiddlePunctMinWidthRatio #1
  {
    ...
    \fp_set:Nn \l_@@_mid_punct_removable_width_ratio_fp
      { ( 1 - (#1) ) / 2 }
  }
...
\cs_new_protected:Npn \SetPunctKernWidthRatio #1
  {
    \fp_set:Nn \l_@@_punct_kern_width_ratio_fp {#1}
  }
...
\tl_new:N \l_@@_punct_offset_dim_tl
\cs_new_protected:Npn \SetPunctOffsetDim #1
  {
    \tl_set:Nn \l_@@_punct_offset_dim_tl {#1}
  }
\SetPunctOffsetDim { 0em }
...
\cs_new_protected:Npn \SetCJKCondensedRatio #1
  {
    \str_if_in:nnTF {#1} { . }
      {
        \msg_error:nnx { newxeCJK } { decimal-not-allowed }
          { \tl_to_str:n {#1} }
      }
      {
...
\cs_new_protected:Npn \@@_assign_charclass_to_charcode:Nn #1#2
  {
    \clist_map_inline:nn {#2}
      {
        \exp_after:wN \@@_assign_charclass_to_charcode_aux:Nw
          \exp_after:wN #1
          \tl_to_str:n {##1} - - \q_stop
      }
  }
    ...
    \tl_if_empty:nTF {#3}
      { \tex_XeTeXcharclass:D \l_tmpa_int = #1 }
      {
        \int_compare:nNnTF \l_tmpa_int > {#3}
          {
            \msg_error:nnxx { newxeCJK } { invalid-range }
...
\@@_assign_charclass_to_charcode:nn { CJK }
  {
